...
<br>
Blind SQLi, as its name says, is a situation where the part of the application that's vulnerable to SQLi doesn't return any relevant response from the database for the SQL Query, so it's not easy to know if that part of the app is actually vulnerable. Typically, these SQLi's are a bit harder to exploit, but not impossible. There are techniques to exploit these cases too.

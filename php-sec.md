...
Type Juggling is a dangerous feature in PHP that can create opening for a vulnerability if the developer is not careful when comparing values. In PHP there are two comparison modes, loose <code>==</code> and strict<code>===</code>.
